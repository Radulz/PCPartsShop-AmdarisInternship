...
const GetMOBOForm = () => {
  ...
  console.log(errors);
  const [mobo, setMobo] = useState(null);
  const notify = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const response = await axios
      .get(process.env.REACT_APP_API_URL + `MOBO/${data.componentId}`)
      .catch((e) => console.log(e));
    console.log(response);
    if (response.data) {
      setMobo(response.data);
    } else {
      notify();
    }
  };
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <br />
      {mobo ? <MOBOCard product={mobo} setProduct={setMobo} /> : null}
    </form>
  );
};

export default GetMOBOForm;
